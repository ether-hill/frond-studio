...
const RACK: RackEntry[] = [
  {
    no: "01", slug: "juno-106", name: "Juno-106", kind: "6-VOICE POLYSYNTH",
  ...
  {
    no: "02", slug: "space-echo", name: "Space Echo", kind: "RE-201 TAPE DELAY",
...
grid.innerHTML = RACK.map((r) =>
  `<a class="rack-card" href="/projects/instruments/${r.slug}" aria-label="Open ${r.name}">` +
    `<div style="display:flex;justify-content:space-between;align-items:baseline;gap:14px">` +
      `<span style="font-family:${MONO};font-size:11px;letter-spacing:0.16em;color:var(--fg4)">${r.no} · ${r.kind}</span>` +
    `</div>` +
